...
public class CommonProvide {
    ...
    public String queryStoreAll(@Param("sidx") String sidx, @Param("sord") String sord,
        ...
        StringBuilder sql=new StringBuilder();
        sql.append(" select s.*, a.name as appname from t_store s left join t_app a on a.id = s.appid where 1=1  and s.status = 0");
    ...
    public String queryStoreAllCount(Store store){
        StringBuilder sql=new StringBuilder();
        sql.append(" select count(s.id) from t_store s left join t_app a on a.id = s.appid where 1=1 and s.status = 0");
    ...
    public String queryProductAll(@Param("sidx") String sidx, @Param("sord") String sord,
        ...
        StringBuilder sql=new StringBuilder();
        sql.append(" select a.name as appname,s.name as storename, p.* from t_product p left join t_store s on p.sid = s.id left join t_app a on a.id = p.appid  where 1=1 and p.status = 0");
    ...
    public String queryProductAllCount(Product product){
        StringBuilder sql=new StringBuilder();
        sql.append(" select count(p.id) from t_product p left join t_store s on p.sid = s.id left join t_app a on a.id = p.appid  where 1=1 and p.status = 0");
